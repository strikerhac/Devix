...
class Parse(object):
    ...
    def executeUsingShellWithConn(self, ip,remote_conn,command_list):
        try:
            ...
            for x in command_list:
                print('Sending command: '+x.get('command'))
                ...
                if formattedOP !='':
                    outputs.append({x.get('command'):self.parse_output(formattedOP, x.get('template'))})
    ...
    def parse_output(self, raw_text_data, template):
    ...
    def perform(self, ip, username, password, command_list):
        try:
            ...
            # termlength = {"command":"yes\n","sleep":2}
            sh_host_name = {"command":"show version \n","sleep":10, "template":"prime_show_version"}
            termlength = {"command":"\n","sleep":2}
            sh_lldp_entry = {"command":"show inventory \n","sleep":10, "template":"prime_show_inventory"}
#             sh_intf = {"command":"sh interfaces x/y/z | i Hardware\n","sleep":5}
            
            
            command_list.append(sh_host_name)
            command_list.append(termlength) 
            command_list.append(sh_lldp_entry)
